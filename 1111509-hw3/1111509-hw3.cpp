...
public:
    ...
    vector(const vector& right)
        : myData()
    {
        size_type size = right.end() - right.begin();
        myData.myFirst = new value_type[size]();
        myData.myLast = myData.myEnd = myData.myFirst + size;

        for (int i = 0; i < myData.myLast - myData.myFirst; i++) {
            myData.myFirst[i] = right.begin()[i];
        }
    ...
    vector& assign(const vector& right)
    {
        if (this != &right) // avoid self-assignment
        {
            ...
            //memcpy(myData.myFirst, right.myData.myFirst, rightSize);
    ...
    bool equal(const vector& right) const
    {
        ...
        for (int i = size() - 1; i >= 0; i--) {
            if (this->myData.myFirst[i] != right.myData.myFirst[i]) {
                return false;
            }
        }
        return true;
    ...
    iterator erase(const_iterator where)
    {
        if (where >= myData.myFirst && where < myData.myLast)
        {
            ...
            size_type index = where - myData.myFirst;
            //memcpy(myData.myFirst + index, myData.myFirst + index + 1, size() - index - 1);
...
HugeInteger HugeInteger::square()
{
    ...
    if (square.integer.begin()[square.integer.size() - 1] == 0)
...
HugeInteger HugeInteger::squareRoot()
{
    ...
    HugeInteger Root((integer.size() + 1) / 2);
    for (int i = Root.integer.size() - 1; i >= 0; i--) {
        int low = 1;
        int high = 999;
        int middle;
        while (low <= high) {
            middle = (low + high) / 2;
            Root.integer.begin()[i] = middle;
            HugeInteger buffer = Root.square();  
            if (equal(buffer))
                return Root;
            if (less(buffer))
                high = middle - 1;

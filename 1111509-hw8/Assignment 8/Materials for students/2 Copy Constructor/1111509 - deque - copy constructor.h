...
public:
      ...
      : myData()
   {
       if (right.myData.mySize > 0)
       {
           ...
           /*for (size_t i = 0; i < myData.mySize / dequeSize; i++) {
               myData.map[i] = new value_type[dequeSize]();
           }

           for (size_t i = 0; i < myData.mySize; i++) {
			   if (right.myData.map[(right.myData.myOff + i) / dequeSize] != nullptr )
			       myData.map[i / dequeSize][i % dequeSize] 
                   = right.myData.map[(right.myData.myOff + i) / dequeSize][(right.myData.myOff + i) % dequeSize];
           }*/

           size_type row = myData.mySize / dequeSize;
		   if (row * dequeSize < myData.mySize)
			   row++;
           ...
           for (int i = row; i < myData.mapSize; i++)

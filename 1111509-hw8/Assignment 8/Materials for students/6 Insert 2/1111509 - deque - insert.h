...
public:
   ...
   reference operator*() const
   {
      ...
      size_type dequeSize = sizeof(value_type) <= 1 ? 16 : sizeof(value_type) <= 2 ? 8
...
public:
   ...
   iterator insert(const_iterator where, const value_type& val)
   {
       ...
       if (myData.mySize == 0)
       {
           ...
           myData.myOff = (off - 1) % (myData.mapSize * dequeSize); 
           size_type block = getBlock(myData.myOff);
           myData.map[block] = new value_type[dequeSize]();
           myData.map[myData.myOff / dequeSize][myData.myOff % dequeSize] = val;

       }
       else
       {
           size_type capacity = myData.mapSize * dequeSize;
           ...
           if (off <= myData.mySize / 2)
           {
               ...
               pointer temp = new value_type[myData.mySize + 1](); 

               iterator it = begin();
               size_type i = 0;
          
               for (; it != begin() + off; i++, it++)
                   temp[i] = *it; 
               temp[i++] = val;
               for (; it != end(); i++, it++)
                   temp[i] = *it;

               myData.myOff = (myData.myOff - 1) % (myData.mapSize * dequeSize);

               size_type block = getBlock(myData.myOff);
               if (myData.myOff % dequeSize == dequeSize - 1) 
                   myData.map[block] = new value_type[dequeSize]();
           ...
           else  // all elements after (and at) where move backward
           {
               ...
               size_type block = getBlock(myData.myOff + myData.mySize + 1);

               if (myData.map[block] == nullptr)
                   myData.map[block] = new value_type[dequeSize]();
               
               for (it = begin(), i = 0; it != end() + 1; i++, it++)
                   *it = temp[i];
...
private:
   ...
   void doubleMapSize()
   {
       if (myData.mapSize > 0)
       {
           ...
           size_type block = myData.myOff / dequeSize;
           for (; block <= (myData.myOff + myData.mySize - 1) / dequeSize; block++)
               newMap[block] = myData.map[block % oldMapSize];

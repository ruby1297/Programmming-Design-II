...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    ...
    for (auto it1 = left.begin(), it2 = right.begin(); it1 != left.end(); it1 = it1->next, it2 = it2->next)
...
public:
   ...
   HugeInteger operator-( const HugeInteger &op2 )
   {
      ...
      auto it2 = op2.integer.begin();
        ...
        for (; it2 != op2.integer.end(); it1 = it1->next, it2 = it2->next) {
            if (it1->myVal < it2->myVal) {
                auto temp = it1->next;
	  ...
	  while (difference.integer.end()->prev->myVal == 0 && !difference.isZero())
   ...
   HugeInteger operator*( HugeInteger &op2 )
   {
	  ...
	  for (; it2 != op2.integer.end(); it2 = it2->next, it3 = it3->next) {
          auto temp = it3;
		  for (it1 = integer.begin(); it1 != integer.end(); it1 = it1->next, temp = temp->next)
			  temp->myVal += it1->myVal * it2->myVal;

	  }
      
	  for (it3 = product.integer.begin(); it3 != product.integer.end(); it3 = it3->next) {
		  if (it3->myVal > 9) {
			  it3->next->myVal += it3->myVal / 10;
			  it3->myVal %= 10;
		  }
	  }
      
    
	  while (product.integer.end()->prev->myVal == 0 && !product.isZero())
   ...
   HugeInteger operator/( HugeInteger &op2 )
   {
      ...
      typename T::iterator it2 = op2.integer.begin();
      int quotientsize = integer.size() - op2.integer.size();
  
      int n = integer.size() - op2.integer.size();

      for (int i = 0; i != n; i++)
          it_buffer = it_buffer->next;

      for (; it2 != op2.integer.end(); it_buffer=it_buffer->next, it2=it2->next)
          it_buffer->myVal = it2->myVal;
      ...
      typename T::iterator it_quotient = quotient.integer.end()->prev;
      for (; quotientsize--; it_quotient = it_quotient->prev) {

...
public:
   ...
   HugeInteger operator-( const HugeInteger &op2 )
   {
      ...
      for (unsigned int i = 0; i < difference.integer.size(); i++, it1++)
          temp[i] = *it1;
      
      for (unsigned int i = 0; i < op2.integer.size(); i++, it2++)
          temp[i] -= *it2;
   ...
   HugeInteger operator*( HugeInteger &op2 )
   {
      ...
      for (unsigned int i = 0; i < integer.size(); i++, it2++) {
          it3 = op2.integer.begin();
          for (unsigned int j = 0; j < op2.integer.size(); j++, it3++) {
              temp[i + j] += (*it2) * (*it3);
          }
      }

      for (unsigned int i = 0; i < product.integer.size(); i++, it1++) {
          if (temp[i] > 9) {
              temp[i + 1] += temp[i] / 10;
              temp[i] %= 10;
          }
